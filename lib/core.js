...
hook('fetch', function(fetch) {
  var fetchTextFromURL = function(url, fulfill, reject) {
    var error = new interop.Reference(NSError, false);
    var moduleSource = NSString.stringWithContentsOfFileEncodingError(url, NSUTF8StringEncoding, error).description;
    if(error.value != null) {
      fulfill(moduleSource);
    } else {
      reject(error.value.domain);
    }
  }

...
hook('normalize', function() {
  ...
  return function(name, parentName) {
    ...
    return new URL(name, coreModulesRoot).href;
  };
});

/*
  NativeScript for iOS specific hook. It locates modules in the file system.
*/
hook('locate', function(locate) {
  var fileManager = NSFileManager.defaultManager();
  return function(load) {
    var loadNameAsNsString = NSString.stringWithString(load.name);
    var isDirectory = new interop.Reference(interop.types.bool, false);

    if (fileManager.fileExistsAtPathIsDirectory(load.name, isDirectory)) {
      if(isDirectory.value) {  // if is folder - search for main file in the folder
        var packageJsonPath = loadNameAsNsString.stringByAppendingPathComponent("package.json");
        var packageJson = NSString.stringWithContentsOfFileEncodingError(packageJsonPath, NSUTF8StringEncoding, null);
        if(packageJson) {
          try {
            var jsFile = JSON.parse(packageJson).main || "index.js";
            return loadNameAsNsString.stringByAppendingPathComponent(jsFile);
          } catch (e) {
            throw new Error("Error parsing package.json in '" + packageJsonPath + "' - " + e);
          }
        }
      ...
      var jsFilePath = loadNameAsNsString.stringByAppendingPathExtension("js");
      if (fileManager.fileExistsAtPathIsDirectory(jsFilePath, isDirectory)) {
        if (isDirectory.value) {
          throw new Error("Expected '" + jsFilePath + "' to be a file");
        }
        return jsFilePath;
      } else {
          throw new Error("Failed to locate module '" + load.name + "'");
      }
    }
  };
});
